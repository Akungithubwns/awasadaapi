    ...
    apiSQL.query("SELECT * FROM `restkey` WHERE `email` = ?", email, function(err, result) {
        ...
        apiSQL.query("SELECT * FROM `restkey` WHERE `username` = ?", userName, function(err, result) {
            ...
            transporter.sendMail({
                from: `"ROHMANZX || REST APIKEY🔥" <${process.env.mail_FROM}>`, // sender address
                to: `"${userName}" <${email}>`, // list of receivers
                subject: "ROHMANZX | Email Verification", // Subject line
                text: "Rohmanzx - Restful 🔥", // plain text body
                html: `
                <p>Hallo ${firstName} ${lastName}</p></br>
                <p>Congratulations! You will soon bind this email address to your ROHMANZX - Restful API account.</p></br>
                ...
                <p>Regards, ROHMANZX</p>
                `, // html body
            }, (err) => {
                if (err) return res.render('register', { message: "Error on mail sender!", messageClass: 'red' })
                apiSQL.query("INSERT INTO `restkey` SET?", { updateAt: mysqlTimestamps, createdAt: mysqlTimestamps, firstname: firstName, lastname: lastName, username: userName, email: email, password: hashedPassword, secretToken: TokenNya, apiKey: 'mannzx'+ crypto.randomBytes(Math.floor(Math.random() * 2) +5).toString('hex'), recovery: 'success', active: 'false' }, function(err, result) {
                    if (err) return res.render('register', { message: "Error on database!", messageClass: 'red' })
...
app.post('/user/recovery', async(req, res) => {
	if (req.body.email) {
		...
		apiSQL.query("SELECT * FROM `restkey` WHERE `email` = ?", req.body.email, function(err, result) {
			...
			const TokenNya = crypto.randomBytes(35).toString('hex')
			transporter.sendMail({
				from: `"ROHMANZX || REAST🔥" <${process.env.mail_FROM}>`, // sender address
				to: `"${result[0].username}" <${req.body.email}>`, // list of receivers
				subject: "ROHMANZX | RESET PASSWORD", // Subject line
				text: "ROHMANZX - Restful API", // plain text body
				html: `
